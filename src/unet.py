...
class EncoderBlock(nn.Module):        
    ...
    def __init__(self, in_chans, out_chans, layers=2, sampling_factor=2, padding="same", batch_norm = False):
        super().__init__()
        self.encoder = nn.ModuleList()
        self.encoder.append(nn.Conv2d(in_chans, out_chans, 3, 1, padding=padding))
        self.encoder.append(nn.ReLU())
        if batch_norm:
            self.encoder.append(nn.BatchNorm2d(out_chans))
...
class UNet(nn.Module):
    def __init__(self, nclass=2, in_chans=3, depth=5, layers=2, sampling_factor=2, skip_connection=True, padding="same", batch_norm=False):
        super().__init__()
        self.encoder = nn.ModuleList()
        self.decoder = nn.ModuleList()
        ...
        for _ in range(depth):
            self.encoder.append(EncoderBlock(in_chans, out_chans, layers, sampling_factor, padding, batch_norm))
